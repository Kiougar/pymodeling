...
class CheckScope(ContextDecorator):
    ...
    def catches(self, exc_type, exc):
        ...
        """
        return (
            (exc_type in self.suppress_types and
             (
    ...
    def __exit__(self, exc_type, exc, tb):
        try:
            ...
            if self.parent:
                self.parent.success = self.parent.success and self.success
            assert scope_stack[-1] is self
            # TODO function pop doesn't return anything...
...
class Process(CheckScope):
    ...
    def __init__(self, name=None, logger=None):
        ...
        # g Added this to stop pycharm from complaining that we define own_logger outside __init__
        self.own_logger = None
        self.handlers = set()

    def add_scope_handler(self, handler):
        """
    ...
    # TODO this function overrides CheckScope.logger, where the latter simply returns the logger, while this sets it
...
class Validation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type is Validation.Abort or self.failures >= self.max_failures:
            if self.failures == 0 and self.enter == 0:
                self.failures += 1
